...
A standard import with `read_csv` for a 600 MB file such as 2008.csv will take a long time to load.  You can see example code below.  The code is not executed becuase of the time delay.
...
However, this does not load faster than `fread()`.  Below is an example with the non-executed code below.
...
The following sections show methods of sampling the data so that you can work with representative subsets of data.  Using dplyr::`sample_frac()` can be an effective way to decrease your elapsed invesitgation times by decreasing computational processing.  `sample_frac` makes it easier to select random rows from a table.
...
The [`microbenchmark` package](https://cran.r-project.org/web/packages/microbenchmark/) can help you estimate how long your processes will take after working with the sample subset.  Another benchmarking alternative is the `rbenchmark` package.
