...
- Supports Publishing through RStudio hosting
    ...
    - [RPubs](https://rpubs.com/) | [Getting Started](https://rpubs.com/about/getting-started)
    - RStudio Connect (teams)

    - Or host your reports other places
...
knitr::include_graphics('Expanding-Brain_tidyverse.jpg')
```
...
Implemented via [R Notebooks](https://bookdown.org/yihui/rmarkdown/notebook.html), an example of an [R Markdown](https://bookdown.org/yihui/rmarkdown/) document, edited within the RStudio IDE.
...
`filter(starwars, eye_color == "orange")`
...
`arrange(starwars, desc(eye_color))`

`arrange(starwars, desc(eye_color), hair_color)`
```
...
select(starwars, name:mass, 10, 7, 4:6)
```
...
```
mutate(starwars, big_mass = mass * 100)
